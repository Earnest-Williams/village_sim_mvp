...
A headless-first survival simulation MVP for a low-graphics medieval village simulator.

This version keeps the simulation core lightweight and uses MessagePack for binary persistence. The runtime now uses typed, data-oriented NumPy-backed state buffers with optional Numba acceleration and is designed to be easy to migrate later to Mojo, Rust, Godot, Bevy, or another renderer/training system.
...
- Deterministic cold-weather model with shelter awareness, cave sheltering behavior, and cold-status event tracking.
- Seeded discoverables (`spring_001`, `berry_bush_001`, `cave_001`) plus discoverable memory.
- Bounded GOAP planning/execution for urgent discoverable needs with utility-policy fallback.
- Trajectory recording plus in-memory action synthesis and persisted action-library I/O.
- Headless deterministic simulation from seed and config.
- ASCII debug map.
- wxPython GUI with semantic colored glyph rendering and stable scrolling during live updates.
- MessagePack run output with metrics, events, and optional snapshots.
- Benchmark runner with timing split categories (`environment`, `perception`, `policy/pathing`, `goap`, `snapshots`, `result aggregation`).
...
```bash
PYTHONPATH=src python -m village_sim.bench --seeds 10 --days 2 --width 32 --height 32
```
