...
class VillageSimFrame(wx.Frame):
    def __init__(self) -> None:
        ...
        controls_sizer = wx.FlexGridSizer(rows=2, cols=8, vgap=8, hgap=8)
        controls_sizer.AddGrowableCol(1, 1)
        controls_sizer.AddGrowableCol(3, 1)
        controls_sizer.AddGrowableCol(5, 1)
        controls_sizer.AddGrowableCol(7, 1)
        ...
        run_button = wx.Button(panel, label="Run Simulation")
        ...
        self.map_ctrl = wx.TextCtrl(panel, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_DONTWRAP)
        self.map_ctrl.SetFont(wx.Font(10, wx.FONTFAMILY_TELETYPE, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))

        root_sizer.Add(controls_sizer, 0, wx.EXPAND | wx.ALL, 12)
        root_sizer.Add(run_button, 0, wx.LEFT | wx.RIGHT | wx.BOTTOM, 12)
    ...
    def on_run(self, _: wx.CommandEvent) -> None:
        ...
        self.summary_ctrl.SetValue(self._format_result(result, len(sim.events)))
        self.map_ctrl.SetValue(render_ascii_map(sim.world, sim.agent))
